...
struct OAuthProviderConfigView: View {
    @State private var githubClientID = UserDefaults.standard.string(forKey: "Lokalo.github.clientID") ?? ""
    @State private var googleClientID = UserDefaults.standard.string(forKey: "Lokalo.googleDrive.clientID") ?? ""
    @State private var onedriveClientID = UserDefaults.standard.string(forKey: "Lokalo.oneDrive.clientID") ?? ""
    ...
    var body: some View {
        Form {
            Section("GitHub") {
                ...
                Text("Erstelle eine OAuth App auf github.com/settings/developers, Callback `it.klincov.lokalo://oauth-callback`, Scope: `public_repo read:user`. Lokalo nutzt den Device-Flow, also kein Client-Secret nötig.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section("Google Drive") {
                TextField("Google iOS Client-ID", text: $googleClientID)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                Text("In console.cloud.google.com → APIs & Services → Credentials → iOS Client ID. Bundle: `it.klincov.lokalo`. Scope: `drive.readonly`.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section("OneDrive / Microsoft Graph") {
                TextField("Microsoft App Client-ID (UUID)", text: $onedriveClientID)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                Text("In portal.azure.com → App registrations → \"Mobile and desktop\" Platform → Redirect URI `it.klincov.lokalo://oauth-callback`. Scopes: `Files.Read offline_access User.Read`.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            Section {
    ...
    private func save() {
        UserDefaults.standard.set(githubClientID.trimmingCharacters(in: .whitespaces), forKey: "Lokalo.github.clientID")
        UserDefaults.standard.set(googleClientID.trimmingCharacters(in: .whitespaces), forKey: "Lokalo.googleDrive.clientID")
        UserDefaults.standard.set(onedriveClientID.trimmingCharacters(in: .whitespaces), forKey: "Lokalo.oneDrive.clientID")
    }
}
