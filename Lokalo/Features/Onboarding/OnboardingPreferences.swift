...
enum OnboardingPreferences {
    ...
    static let hasCompletedKey = "Lokalo.hasCompletedOnboarding"

    /// Allow large model downloads over cellular. Default `false` → WLAN-only.
    static let cellularDownloadsAllowedKey = "Lokalo.onboarding.cellularDownloadsAllowed"
    ...
    static let preferredFirstModelIDKey = "Lokalo.onboarding.preferredFirstModelID"
    ...
    static let allowBackgroundActivityKey = "Lokalo.settings.allowBackgroundActivity"

    /// Dark / Light appearance preference. Stored as the raw string of
    /// `AppearanceMode`. `LokaloApp` reads this key via `@AppStorage`
    ...
    static let appearanceModeKey = "Lokalo.onboarding.appearanceMode"
