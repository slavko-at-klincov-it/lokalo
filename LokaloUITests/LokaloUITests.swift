...
final class LokaloUITests: XCTestCase {
    ...
    private func launchApp(extraArgs: [String] = []) -> XCUIApplication {
        ...
        app.launchArguments = ["-Lokalo.hasCompletedOnboarding", "1"] + extraArgs
        app.launch()
        return app
    }
    ...
    func testFullChatRoundTrip() {
        let app = launchApp(extraArgs: ["-LokaloAutoTestPrompt", "Sage einfach: Hallo"])
        ...
        while Date() < deadline {
            let assistantTexts = app.staticTexts.allElementsBoundByIndex.filter { e in
                ...
                return !l.isEmpty && !l.contains("Sage einfach") && l.count > 1
                    && !l.contains("Lokalo") && !l.contains("Qwen") && !l.contains("Llama")
